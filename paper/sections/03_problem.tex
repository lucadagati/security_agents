...
directed graph $G_t=(V_t,E_t)$~\cite{park2023generative}; coalitions are
detected \emph{post hoc} from reciprocal same-role traffic and are never
objectives in themselves~\cite{abdelnabi2024cooperation}.

Table~\ref{tab:rq} lists the research questions. RQ0--RQ3 and RQ5 are
evaluated in Section~\ref{sec:results} on one-versus-one cells; RQ4
(population scaling) is specified in the experimental matrix but left for
future work. A behaviour is \emph{emergent} when it is not encoded in
objectives or declared policy class yet appears consistently across
episodes and seeds; post-hoc tagging produces candidates for emergence,
...
RQ2 & Emergent behaviours vs.\ encoded set $B_i^{\mathrm{enc}}$ \\
RQ3 & Co-evolution measured by CEP and strategy signatures \\
RQ4 & Population size, topology, communication (E4--E8; future) \\
RQ5 & Dynamics class: equilibrium, convergence, arms race \\
\bottomrule
\end{tabularx}
\end{table}

Each turn proceeds as a simultaneous move with environment-mediated
feedback: agents act in a fixed order (attacker then defender within the
turn), but neither observes the other's internal state. Episode termination
